/* Bryan Resner - CS362 Spring 2017 - Refactored Code */
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
            return playAdventurer(state, &drawntreasure, currentPlayer, temphand, handPos);
...
int playAdventurer(struct gameState *state, int *drawntreasure,
                   int currentPlayer, int *temphand, int handPos) {
    int z = 0;// this is the counter for the temp hand
    int cardDrawn;
    while((*drawntreasure)<2){
        if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
        ...
        if (drawCard(currentPlayer, state) < 0) {
            //The deck and discard pile are empty
            break;
        }
        cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
        if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
    ...
    //!!!! Added this code to discard adventurer after it has been played
    discardCard(handPos, currentPlayer, state, 0);
    return 0;
}
...
int playSteward(struct gameState *state, int currentPlayer, int handPos, int choice1,
                int choice2, int choice3) {
    if (choice1 == 1)
    {
        //+2 cards
        drawCard(currentPlayer, state);
